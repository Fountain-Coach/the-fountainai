...
## Setup
- Start FountainAiLauncher (CLI or GUI) so Planner, Function Caller, and gateway URLs from **_includes/env.md** are provided automatically for local runs.
...
## Runbook
1. Bring the supervisor up with `bash Scripts/launcher start` (or the GUI) and ensure the control plane reports HTTP 200 before launching the dashboard.
...
7. Quit (`q`), review `bash Scripts/launcher logs -f` for paired `/planner` and `/planner/execute` calls plus any surfaced `NotSupported` capabilities, then `bash Scripts/launcher stop` when done.
