...
- Keep FountainAiLauncher running so baseline, drift, and reflections endpoints resolve automatically using the signed environment from **_includes/env.md**.
...
## Runbook
1. Start FountainAiLauncher via `bash Scripts/launcher start` (or the GUI) and ensure the control plane reports HTTP 200; keep the corpus id created in Module 02 handy.
...
7. Quit with `q` and follow `bash Scripts/launcher logs -f` for successful `/baselines`, `/drift`, and `/reflections` calls, then stop the launcher with `bash Scripts/launcher stop` when finished.
