...
private struct Outcome: Codable { let steps: [String: String] }
...
private func run() async {
    ...
    do {
        _ = try await persist.createCorpus(.init(corpusId: corpusId))
    ...
    // Add baseline
    do {
        let ack = try await persist.addBaseline(
            corpusId: corpusId,
            baseline: .init(baselineId: baselineId, corpusId: corpusId, content: baselineContent)
        )
    ...
    // Add reflection
    do {
        let ack = try await persist.addReflection(
            corpusId: corpusId,
            reflection: .init(reflectionId: reflectionId, corpusId: corpusId, question: reflectionQ, content: reflectionA)
        )
...
// Top-level entry point using Task to support async work
