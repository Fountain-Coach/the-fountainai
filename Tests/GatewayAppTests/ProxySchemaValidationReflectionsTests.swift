...
final class ProxySchemaValidationReflectionsTests: XCTestCase {
    @MainActor
    func testReflectionsSummaryViaGatewayMatchesSchema() async throws {
        ...
        _ = try await router.route(.init(method: "POST", path: "/corpus/reflections", body: try JSONEncoder().encode(ReflectionRequest(corpusId: "gwr", reflectionId: "r1", question: "q", content: "a"))))
        ...
        let (data, resp) = try await URLSession.shared.data(from: URL(string: "http://127.0.0.1:\(port)/awareness/corpus/reflections/gwr")!)
        ...
        guard let rs = schemas?["ReflectionSummaryResponse"] as? [String: Any] else {
            return XCTFail("ReflectionSummaryResponse missing in spec")
        }
        XCTAssertTrue(OpenAPISchemaValidator.validate(object: obj ?? [:], against: rs))
